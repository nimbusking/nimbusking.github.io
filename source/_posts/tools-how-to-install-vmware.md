...
> 💡 如何确认虚拟化是否已启用？
>
> 在 Windows 中打开**任务管理器**（`Ctrl + Shift + Esc`），切换到"**性能**"标签页，选中"**CPU**"，查看右下角的"**虚拟化**"状态。如果显示"已启用"，则说明 BIOS 设置正确。如下图所示：
> ![虚拟化打开](65cd2699/virtual_on.png)

### 汉化文件准备
这里我具体不列举，如果你真的有需要，单独网页检索一下汉化的方式。
网上提供的汉化方式，我没有测试过。这里列出来只是告知，确实有汉化的方式。
...
如下图所示：
![注册页面](65cd2699/broadcom_registration_web.png)

### 填写注册信息

*PS：我很久之前就注册过了，我就不单独再次注册截图了*
...
## 第二步：下载 VMware Workstation Pro 26H1
...
如下图所示：
![下载入口](65cd2699/download_click.png)

### 选择版本并下载

1. 在免费下载页面找到 **"VMware Workstation Pro"** 并点击
    直接找右下角 
    ![下载链接](65cd2699/vmware_workstation_pro_download_link.png)
2. 选择最新版本 **"VMware Workstation Pro 26H1 for Windows"**（Windows 用户）或 **"for Linux"**（Linux 用户）
    ![下载版本](65cd2699/download_version.png)
    点击具体版本页面
    ![点击版本](65cd2699/click_download_page.png)
...
> 📦 **下载文件名示例**：
> - Windows：`VMware-Workstation-Full-26H1-25388281.exe`（约 274.34 MB）
> - Linux：`VMware-Workstation-Full-26H1-25388281.x86_64.bundle`（约 325.03 MB）
...
![Windows 功能关闭](65cd2699/windows_optionalfeatures.png)
...
### 关闭 Device Guard / Credential Guard（如适用）
...
你可能听说过 VMware Workstation 可以配合 Windows 的 **VBS（基于虚拟化的安全）** 使用。自 Workstation Pro 版本后，VMware 引入了对 **Windows Hypervisor Platform (WHP)** 的支持，使得在一定程度上可以与 Hyper-V 共存。
...
下面我们将详细介绍如何创建一台 Windows 11 虚拟机。其他windows操作系统的流程大同小异，主要区别在于选择合适的客户操作系统类型和硬件配置。
linux相关，等我有空的时候，单独补充一下
...
| **Custom (advanced)** | 自定义（高级），完全控制所有硬件参数 | 有经验的用户，需要精细控制，下文中有一个异常场景需要用到这个模式安装 |
...
### 虚拟机启动报错："VMware Workstation and Hyper-V are not compatible"

**原因**：Windows Hyper-V 仍在启用状态。

**解决方法**：
1. 按第 节所述方法关闭 Hyper-V 及相关功能
2. 以管理员身份运行命令提示符，执行：
   ```shell
...
### Windows 11 安装提示"此电脑不符合安装此版本 Windows 的最低系统要求"
