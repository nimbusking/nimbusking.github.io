...
- 本文以《从Paxos到ZooKeeper 分布式一致性原理与实践》一书为主要参考，节选书中比较核心的一些知识点。
...
### Paxos
Paxos算法是莱斯利·兰伯特( [Leslie Lamport](https://en.wikipedia.org/wiki/Leslie_Lamport)) （【备注】计算机界的一位巨佬，其在分布式领域的贡献不亚于Dijkstra这些巨佬的贡献，2013年图灵奖获得者）1990年提出的一种基于消息传递且具有高度容错特性的一致性算法，是目前公认的解决分布式一致性问题最有效的算法之一。
...
- **ACL：保障数据的安全**
### 序列化与协议-Jute
PS: 知道有这么个东西，ZooKeeper官方团队曾经一直想换，但是种种原因没换成，最后沿用到现在，Jute也不是什么大问题，够用。
### 客户端
