...
<!-- more -->
![Java并发知识体系](post/181e5700/Java并发知识体系.jpg)
...
![JMM内存模型示意图](post/181e5700/JMM内存模型示意图.png)
#### 关联两个疑问
...
![MESI协议状态流转问题](post/181e5700/MESI协议状态流转问题.png)
...
![伪共享内存交互示意图](post/181e5700/伪共享内存交互示意图.png)
上图对应的程序代码片段
```java
class Pointer {
	volatie long x;
...
#### 两者切换的场景
![用户态和内核态切换的场景](post/181e5700/用户态和内核态切换的场景.png)
- 系统调用：调用系统底层API
...
![LongAdder竞争示意图](post/181e5700/LongAdder竞争示意图.png)
...
![Java中的Monitor机制执行流程](post/181e5700/Java中的Monitor机制执行流程.png)
解释起来：
...
![对象头内存占用分布示意图](post/181e5700/对象头内存占用分布.png)

##### 锁对象状态转换
![锁对象状态转换示意图](post/181e5700/锁对象状态转换示意图.png)
##### 锁的优化
...
![AQS-同步等待队列](post/181e5700/AQS-同步等待队列.png)
...
#### Condition接口详解
![Condition接口](post/181e5700/structure_of_condition.png)
...
调用流程大致如下：
*PS：整整画了我2小时 :(*
![完整调用流程](post/181e5700/ReentrantLock执行流程图.jpg)
...
![Semaphore示意图](post/181e5700/Semaphore示意图.jpg)
...
![CountDownLatch示意图](post/181e5700/CountDownLatch示意图.jpg)
...
![CyclicBarrie示意图](post/181e5700/CyclicBarrie示意图.jpg)
...
##### 相关原理
![读写锁类结构](post/181e5700/structure_of_ReentrantReadWriteLock.jpg)
相比其它锁，共有的两个公平非公平同步结构之外，还多了两个单独实现Lock接口的锁模块。
两个锁本质是通过state状态的高低位是否为0来判断是否有读锁/写锁，其中：写锁实现就是独占锁；读锁实现就是共享锁实现。
![ReentrantReadWrite锁工作示意图](post/181e5700/ReentrantReadWrite锁工作示意图.jpg)
...
![读写锁高低位](post/181e5700/读写锁高低位.jpg)
...
![take方法示例](post/181e5700/take方法示例.jpg)
...
![put方法示例](post/181e5700/put方法示例.jpg)
...
![阻塞队列一览](post/181e5700/阻塞队列一览.jpg)
...
原理如下图所示：
![RingBuffer结构示意图](post/181e5700/RingBuffer结构示意图.jpg)
其中：
...
![生产者消费者模式](post/181e5700/生产者消费者模式.jpg)
##### 模式优点
1. **支持异步处理**
例子：用户注册后，需要发注册邮件和注册短信。传统的做法有两种 1.串行的方式；2.并行方式，如下图所示：
![传统模式](post/181e5700/2024-11-02_160057.jpg)
引入消息队列后：
![引入消息队列后](post/181e5700/2024-11-02_160137.jpg)


2. **解耦**
场景：用户下单后，订单系统需要通知库存系统扣减库存。
![解耦](post/181e5700/2024-11-02_160219.jpg)

3. **可以消除生产者生产与消费者消费之间速度差异（削峰填谷）**
例如：
![速度差异的处理](post/181e5700/2024-11-02_160251.jpg)
