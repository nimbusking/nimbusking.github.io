...
所以基本上不用改，唯一需要该的是nodejs版本，因为最新的hexo插件，至少需要node版本在14以上才行。所以，我升级了一下。

其它的没啥了。

### 一些异常
#### local search异常
具体现象就是，在网页端搜索的时候，关键词搜索不到内容，但是确定博客中肯定是有这个关键词的

【解决办法】 你本地运行之后，在你的浏览器中直接输入：http://localhost:4000/search.xml
看看有没有报错，我的就报错了如下图所示：
![运行报错](178a5861/MyCapture_2026-05-15_16-18-52.jpg) 

然后在浏览器中右键将这个search.xml保存到本地，根据这个报错行坐标定位报错行：
![报错行](178a5861/MyCapture_2026-05-15_17-32-36.jpg)

**明显就是这行有问题，你看颜色就能直观看出来**
【问题原因】 这篇文章中用了特殊的xml语法，导致了search.xml存在语法错误了，去文章中修改了就可以了。