...
- 2. **两阶段提交（2PC）**  
  ...
  **Prepare阶段**：事务写入Redo Log并标记为Prepare。  
  **Commit阶段**：写入Binlog后，Redo Log标记为Commit。
- 3. **主从复制原理**  
...
- 1. **复制流程**
   1. 主库将变更写入Binlog。
   2. 从库IO线程拉取Binlog并写入中继日志（Relay Log）。
   3. 从库SQL线程解析Relay Log并应用数据。
- 2. **同步方式**
   - **异步复制**：默认方式，主库不等待从库确认。
   - **半同步复制**：至少一个从库确认后才返回客户端成功。
- 3. **延迟问题**
   - **原因**：从库单线程回放、大事务、网络延迟。
   - **解决**：并行复制（5.6+）、分库分表减少单库压力。
