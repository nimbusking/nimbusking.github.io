...
![mvcc示例数据](learning-mysql-how-to-run/mvcc示例数据.jpg)

假设插入该记录的事务id为80，那么此条记录的示意图如下图所示：

![mvcc示例数据结构示意图](learning-mysql-how-to-run/mvcc示例数据结构示意图.jpg)

假设之后两个事务id分别为100、200的事务对这条记录进行update操作，操作流程如下图所示：

![update操作流程](learning-mysql-how-to-run/update操作流程.jpg)

由于每对记录进行一次改动，都会记录一条undo日志。每条undo日志也都有一条roll_pointer属性，通过这个属性可以将这些undo日志串联成一个链表，所以现在的情况就如下图所示：

![undo日志串成一个链表](learning-mysql-how-to-run/undo日志串成一个链表.jpg)
